/// @file tests_oversampler.cc
/// @brief SecondOrderRaw filters specific tests
/// @author gm
/// @copyright gm 2014
///
...
static const float kPassthroughFrequency(
  Oversampler<SecondOrderRaw>::Meta().freq_passthrough);
/// @brief Resonance parameter to be set in order to have a near-passthrough
static const float kPassthroughResonance(
  Oversampler<SecondOrderRaw>::Meta().res_passthrough);

/// @brief Random distribution for filter frequency, within its own bounds
std::uniform_real_distribution<float> OverSamplerSecondOrderRawFilterFreqDistribution(
  Oversampler<SecondOrderRaw>::Meta().freq_min,
  Oversampler<SecondOrderRaw>::Meta().freq_max);

/// @brief Filters a random signal, check for mean lower than the one
/// of the input signal (no DC offset introduced)
TEST(Filters, OverSamplerSecondOrderRawOutputMean) {
  for (unsigned int iterations(0); iterations < kIterations; ++iterations) {
    IGNORE(iterations);

    // Random normalized frequency
    const float kFrequency(OverSamplerSecondOrderRawFilterFreqDistribution(kRandomGenerator));
    Oversampler<SecondOrderRaw> filter;
...
TEST(Filters, OverSamplerSecondOrderRawPassthrough) {
  Oversampler<SecondOrderRaw> filter;
...
TEST(Filters, OverSamplerSecondOrderRawPerf) {
  for (unsigned int iterations(0); iterations < kIterations; ++iterations) {
    IGNORE(iterations);
