...
static const float kPassthroughFrequency(MoogLowAliasNonLinear::Meta().freq_passthrough);
/// @brief Resonance parameter to be set in order to have a near-passthrough
static const float kPassthroughResonance(MoogLowAliasNonLinear::Meta().res_passthrough);

/// @brief Random distribution for filter frequency, within its own bounds
std::uniform_real_distribution<float> MoogLowAliasNonLinearFilterFreqDistribution(
  MoogLowAliasNonLinear::Meta().freq_min,
  MoogLowAliasNonLinear::Meta().freq_max);

/// @brief Filters a random signal, check for mean lower than the one
/// of the input signal (no DC offset introduced)
TEST(Filters, MoogLowAliasNonLinearOutputMean) {
  for (unsigned int iterations(0); iterations < kIterations; ++iterations) {
    IGNORE(iterations);

    // Random normalized frequency
    const float kFrequency(MoogLowAliasNonLinearFilterFreqDistribution(kRandomGenerator));
...
TEST(Filters, MoogLowAliasNonLinearPerf) {
  for (unsigned int iterations(0); iterations < kIterations; ++iterations) {
    IGNORE(iterations);
