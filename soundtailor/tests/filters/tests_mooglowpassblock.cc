/// @file tests_mooglowpassblock.cc
/// @brief Moog lowpass Moog building block tests
/// @author gm
/// @copyright gm 2014
///
...
static const float kPassthroughFrequency(MoogLowPassBlock::Meta().freq_passthrough);
...
TEST(Filters, MoogLowPassBlockPassthrough) {
  MoogLowPassBlock filter;
...
TEST(Filters, MoogLowPassBlockRange) {
  MoogLowPassBlock filter;
...
TEST(Filters, MoogLowPassBlockPerf) {
  for (unsigned int iterations(0); iterations < kIterations; ++iterations) {
    IGNORE(iterations);
