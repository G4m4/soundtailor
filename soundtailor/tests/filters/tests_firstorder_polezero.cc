/// @file tests_firstorder_polezero.cc
/// @brief SoundTailor 1st order Pole-Zero filters tests
/// @author gm
/// @copyright gm 2014
///
...
TEST(Filters, FirstOrderPoleZeroPassthrough) {
  FirstOrderPoleZero filter;
...
TEST(Filters, FirstOrderPoleZeroRange) {
  FirstOrderPoleZero filter;
...
TEST(Filters, FirstOrderPoleZeroPerf) {
  for (unsigned int iterations(0); iterations < kIterations; ++iterations) {
    IGNORE(iterations);
